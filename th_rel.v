...
Section Tortoise_and_Hare_tail_recursive.
  ...
  (** This version only need a decidable binary relation R. It does
      not requires that R is equality @eq X *)

  Variables (X : Type) 
            (R : X -> X -> Prop)
            (Rdec : forall x y : X, { R x y } + { ~ R x y }).

  Variable (f : X -> X) (x0 : X) 
           (Hx0 : exists τ, 0 < τ /\ R (f↑τ x0) (f↑(2*τ) x0)).

  Inductive bar_tl i x y : Prop :=
    | in_bar_tl_0 : R x y                        -> bar_tl i x y 
    | in_bar_tl_1 : bar_tl (S i) (f x) (f (f y)) -> bar_tl i x y.

  Let tortoise_hare_tail_rec : 
    forall i x y, bar_tl i x y -> { k | i <= k /\ R (f↑(k-i) x) (f↑(2*(k-i)) y) }.
  Proof.
    ...
    * split; auto; eq goal with E; f_equal; replace (i-i) with 0 by omega; reflexivity.
  ...
  Definition tortoise_hare_tail : { τ | 0 < τ /\ R (f↑τ x0) (f↑(2*τ) x0) }.
  Proof.
